...
class PhototourismDataset(Dataset):
    def __init__(self, root_dir, split='train', img_downscale=1, val_num=1, use_cache=False, batch_size=1024, scale_anneal=-1, min_scale=0.25, use_semantics=False, feature_dim=512,features_path="",num_of_img=None,images_file=None):
        """
...
#
        ...
        if self.num_of_img is not None:
            if self.images_file is not None:
                idx_to_use = get_k_files(self.num_of_img, self.images_file)
                idx_to_use = [int(file.split('.')[0]) for file in idx_to_use]
            else:
                idx_to_use=[]
                while len(idx_to_use)<self.num_of_img:
                    id = self.img_ids[random.randint(0,len(self.img_ids)-1)]
                    while (not os.path.isfile(feature_paths[id])) or (id in idx_to_use):
                        id = self.img_ids[random.randint(0,len(self.img_ids)-1)]
                    idx_to_use.append(id)
                print("len(idx_to_use)"+str(len(idx_to_use)))
        ...
        if self.split == 'train': # create buffer of all rays and rgb data
            ...
            self.val_id = self.img_ids_train[9]
    ...
    def __getitem__(self, idx):
        if self.split == 'train': # use data in the buffers
            ...
            if self.use_semantics:
                #dim = 45
                #img_pred = np.clip(self.all_rgbs[rgb_sample_points].detach().view(dim, dim, 3).cpu().numpy(), 0, 1)
                #imgpred = (img_pred * 255).astype(np.uint8)
                #plt.imsave("gt.png",imgpred)
                #with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.float32):
                #    visfeat = feature_utils.feature_vis(self.all_features[rgb_sample_points,:],dim)
                #    plt.imsave("feature.png",visfeat)
